...
int sfp_refresh_thread(void *opaque)
{
	...
	while (!thread_exit) {
		...
		SDL_PushEvent(&event);
		SDL_Delay(5);	//调节了时间，解决SDL界面卡住的问题
	}
	thread_exit=0;
	//Break
	SDL_Event event;
...
void show_dshow_device_option(){
	...
	avformat_open_input(&pFormatCtx,"video=Integrated Webcam",iformat,&options);
	printf("================================\n");
}
...
//Windows
	...
	char * psDevName = dup_wchar_to_utf8(L"audio=麦克风阵列 (Realtek High Definition ");
	//char * psDevName = "audio=麦克风阵列 (Realtek High Definition ";

	if (avformat_open_input(&pFormatCtx, psDevName, ifmt,NULL) < 0)
	{
	...
	//unsigned char *out_buffer=(unsigned char *)av_malloc(avpicture_get_size(AV_PIX_FMT_YUV420P, pVideoCodecCtx->width, pVideoCodecCtx->height));
	...
	FILE *lpPcm = fopen("audio.pcm", "wb+");
#endif  

	struct SwsContext *img_convert_ctx;
	img_convert_ctx = sws_getContext(pVideoCodecCtx->width, pVideoCodecCtx->height, pVideoCodecCtx->pix_fmt, pVideoCodecCtx->width, pVideoCodecCtx->height, AV_PIX_FMT_YUV420P, SWS_BICUBIC, NULL, NULL, NULL); 
	//------------------------------
